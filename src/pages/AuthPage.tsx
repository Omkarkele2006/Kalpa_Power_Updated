...
import { Pencil, Users, Shield, HardHat, Building2 } from 'lucide-react';
import type { Database } from '@/integrations/supabase/types';

type AppRole = Database['public']['Enums']['app_role'];

const roleOptions: { value: AppRole; label: string; icon: typeof Pencil; desc: string }[] = [
  { value: 'designer', label: 'Designer', icon: Pencil, desc: 'Create and submit drawings' },
  { value: 'line-manager', label: 'Line Manager', icon: Users, desc: 'Technical review queue' },
  { value: 'dept-head', label: 'Dept Head', icon: Shield, desc: 'Final approval authority' },
  { value: 'site-engineer', label: 'Site Engineer', icon: HardHat, desc: 'View approved docs & QR verify' },
  { value: 'vendor-client', label: 'Vendor / Client', icon: Building2, desc: 'View final approved documents' },
];
    ...
    try {
      if (isSignUp) {
        ...
        if (data.user) {
          ...
          await supabase.from('profiles').update({ title: roleOptions.find(r => r.value === role)?.desc ?? null }).eq('id', data.user.id);
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="rounded-xl border bg-card p-6 shadow-sm space-y-5">
          <h2 className="text-lg font-semibold text-center">{isSignUp ? 'Create Account' : 'Sign In'}</h2>
